pipeline {
	...
	stage("Deploying to Kubernetes production environment"){
	    steps{
		 ...
		 sh 'kubectl set image -n ben-space deployment/micro-deployment micrometer-demo-deployment=$registry:$currentBuild:latest' //using the terminal, will default to latest tag
